'use client';

import {
    PremiumTable,
    PremiumTableBody,
    PremiumTableCell,
    PremiumTableHead,
    PremiumTableHeader,
    PremiumTableRow,
    BadgeDRC
} from '@/components/ui/PremiumTable';
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
    DropdownMenuLabel,
    DropdownMenuTrigger,
} from '@/components/ui/dropdown-menu';
import { Button } from '@/components/ui/button';
import { MoreHorizontal, Plus, Trash2, Book, FileText, Settings, Database, Search } from 'lucide-react';
import {
    useGetJournalsQuery,
    useDeleteJournalMutation,
} from '../api/accountingApi';
import { Skeleton } from '@/components/ui/skeleton';
import { toast } from 'sonner';
import React, { useState } from 'react';
import { Journal } from '../types';
import { extractArray } from '@/lib/utils';
import { JournalDialog } from './JournalDialog';
import { Input } from '@/components/ui/input';

export const JournalList = () => {
    const { data, isLoading, error } = useGetJournalsQuery();
    const [deleteJournal] = useDeleteJournalMutation();
    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [selectedJournal, setSelectedJournal] = useState<Journal | null>(null);
    const [search, setSearch] = useState('');

    const journals = extractArray<Journal>(data);

    const handleCreate = () => {
        setSelectedJournal(null);
        setIsDialogOpen(true);
    };

    const handleEdit = (journal: Journal) => {
        setSelectedJournal(journal);
        setIsDialogOpen(true);
    };

    const handleDelete = async (id: number) => {
        if (confirm('Êtes-vous sûr de vouloir supprimer ce journal ?')) {
            try {
                await deleteJournal(id).unwrap();
                toast.success('Journal supprimé');
            } catch (err: any) {
                toast.error(err?.data?.message || 'Erreur lors de la suppression');
            }
        }
    };

    if (isLoading) {
        return (
            <div className="space-y-6">
                <div className="flex justify-between">
                    <Skeleton className="h-12 w-1/3" />
                    <Skeleton className="h-12 w-48" />
                </div>
                <Skeleton className="h-64 rounded-3xl" />
            </div>
        );
    }

    if (error) {
        return <div className="text-red-500 font-bold p-8 text-center bg-red-50 rounded-2xl border border-red-100">Erreur lors du chargement des journaux.</div>;
    }

    const filteredJournals = journals.filter(j =>
        j.code.toLowerCase().includes(search.toLowerCase()) ||
        j.label.toLowerCase().includes(search.toLowerCase())
    );

    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 pb-4">
                <div>
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border border-blue-100 text-blue-700 text-[10px] font-black mb-4 uppercase tracking-[0.2em]">
                        <div className="h-1.5 w-1.5 rounded-full bg-drc-blue animate-pulse" />
                        Structure Comptable
                    </div>
                    <h2 className="text-4xl font-black tracking-tight text-slate-900 font-outfit uppercase">Journaux</h2>
                    <p className="text-slate-500 font-medium mt-1">Configurez vos journaux auxiliaires pour la saisie des opérations.</p>
                </div>
                <Button
                    className="h-12 px-6 rounded-xl bg-drc-blue hover:bg-blue-700 text-white font-bold shadow-lg shadow-blue-500/20 transition-all active:scale-95"
                    onClick={handleCreate}
                >
                    <Plus className="mr-2 h-5 w-5" /> Nouveau Journal
                </Button>
            </div>

            <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm">
                <div className="flex items-center gap-4 py-2 mb-4">
                    <div className="relative flex-1 max-w-sm group">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 group-focus-within:text-drc-blue transition-colors" />
                        <Input
                            placeholder="Rechercher par code ou libellé..."
                            className="h-12 pl-12 rounded-xl bg-slate-50 border-transparent focus:bg-white focus:border-slate-100 transition-all font-medium"
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                        />
                    </div>
                </div>

                <div className="rounded-2xl border border-slate-50 overflow-hidden">
                    <PremiumTable>
                        <PremiumTableHeader>
                            <PremiumTableRow className="bg-slate-50/50">
                                <PremiumTableHead>Code Journal</PremiumTableHead>
                                <PremiumTableHead>Libellé Signature</PremiumTableHead>
                                <PremiumTableHead>Types & Catégories</PremiumTableHead>
                                <PremiumTableHead className="w-[80px] text-right">Actions</PremiumTableHead>
                            </PremiumTableRow>
                        </PremiumTableHeader>
                        <PremiumTableBody>
                            {filteredJournals.length === 0 ? (
                                <PremiumTableRow>
                                    <PremiumTableCell colSpan={4} className="text-center py-24 text-slate-400 font-medium italic">
                                        <Book className="h-12 w-12 mx-auto mb-4 opacity-10" />
                                        Aucun journal configuré.
                                    </PremiumTableCell>
                                </PremiumTableRow>
                            ) : (
                                filteredJournals.map((journal) => (
                                    <PremiumTableRow key={journal.id} className="hover:bg-slate-50/50 transition-colors group">
                                        <PremiumTableCell>
                                            <div className="flex items-center gap-3">
                                                <div className="h-10 w-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500 group-hover:bg-drc-blue group-hover:text-white transition-all duration-300">
                                                    <FileText className="h-5 w-5" />
                                                </div>
                                                <div className="font-mono text-xs font-black text-drc-blue uppercase tracking-widest">
                                                    {journal.code}
                                                </div>
                                            </div>
                                        </PremiumTableCell>
                                        <PremiumTableCell>
                                            <div className="font-bold text-slate-900 uppercase">{journal.label}</div>
                                            <div className="text-[10px] text-slate-400 font-medium truncate max-w-[300px] mt-0.5">
                                                {journal.description || 'Aucune description spécifiée'}
                                            </div>
                                        </PremiumTableCell>
                                        <PremiumTableCell>
                                            <BadgeDRC variant="slate">
                                                <Settings className="mr-1 h-3 w-3 inline" /> STANDARD
                                            </BadgeDRC>
                                        </PremiumTableCell>
                                        <PremiumTableCell className="text-right">
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                    <Button variant="ghost" className="h-10 w-10 p-0 rounded-xl hover:bg-white hover:shadow-md transition-all">
                                                        <MoreHorizontal className="h-5 w-5 text-slate-400" />
                                                    </Button>
                                                </DropdownMenuTrigger>
                                                <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl border-slate-100 shadow-2xl">
                                                    <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-3 py-2">Administration Journal</DropdownMenuLabel>
                                                    <DropdownMenuItem onClick={() => handleEdit(journal)} className="focus:bg-slate-50 rounded-xl px-3 py-2.5 font-bold text-slate-700 cursor-pointer">
                                                        MODIFIER CONFIGURATION
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem className="focus:bg-slate-50 rounded-xl px-3 py-2.5 font-bold text-slate-700 cursor-pointer">
                                                        VOIR ÉCRITURES
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem
                                                        className="focus:bg-red-50 text-drc-red font-black rounded-xl px-3 py-2.5 cursor-pointer"
                                                        onClick={() => handleDelete(journal.id)}
                                                    >
                                                        SUPPRIMER JOURNAL
                                                    </DropdownMenuItem>
                                                </DropdownMenuContent>
                                            </DropdownMenu>
                                        </PremiumTableCell>
                                    </PremiumTableRow>
                                ))
                            )}
                        </PremiumTableBody>
                    </PremiumTable>
                </div>
            </div>
            <JournalDialog open={isDialogOpen} onOpenChange={setIsDialogOpen} journalToEdit={selectedJournal} />
        </div>
    );
};
